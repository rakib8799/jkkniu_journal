                ...
                <div class="row justify-content-center">
                    ...
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-body">
                                <div class="mt-2">
                                    <label for="volume"><b>Select Volume: <span class="text-danger">*</span></b></label>
                                    <div class="input-group mt-2">
                                        <br>
                                        <select name="volume" id="volume" class="form-select" required>
                                            <option value="">Please select any volume</option>
                                            <?php
                                            $select_from_new_paper = "SELECT DISTINCT vol_name FROM `journal_volume` ORDER BY `vol_name`";
                                            $run_select_from_new_paper = mysqli_query($conn, $select_from_new_paper);
                                            if (mysqli_num_rows($run_select_from_new_paper) > 0) {
                                                while ($row = mysqli_fetch_assoc($run_select_from_new_paper)) {
                                                    extract($row);
                                            ?>
                                                    <option value="<?php echo $vol_name ?>"><?php echo $vol_name ?></option>
                                            <?php
                                                }
                                            }
                                            ?>
                    ...
                    <div class="col-lg-6">
                        <div class="card">
                            <div class="card-body">
                                <div class="mt-2">
                                    <label for="issue"><b>Select Issue: <span class="text-danger">*</span></b></label>
                                    <div class="input-group mt-2">
                                        <br>
                                        <select name="issue" id="issue" class="form-select" required>
                                            <option value="">Please select any issue</option>
                                            <?php
                                            $select_from_new_paper = "SELECT DISTINCT issue_name FROM `journal_issue` ORDER BY `issue_name`";
                                            $run_select_from_new_paper = mysqli_query($conn, $select_from_new_paper);
                                            if (mysqli_num_rows($run_select_from_new_paper) > 0) {
                                                while ($row = mysqli_fetch_assoc($run_select_from_new_paper)) {
                                                    extract($row);
                                            ?>
                                                    <option value="<?php echo $issue_name ?>"><?php echo $issue_name ?></option>
                                            <?php
                                                }
                                            }
                                            ?>
...
?>
    ...
    if ($count_error > 0) {
        ...
        $insert_qry = "INSERT INTO `new_paper`(`paper_id`, `author_id`, `reviewer_id`, `paper_title`, `paper_abstract`, `paper_keywords`, `paper_type`, `authors_name`, `authors_affiliation`, `authors_designation`, `authors_email`, `manuscript_pdf`, `cover_letter_pdf`, `manuscript_image`, `supplimentary_file`,`volume`,`issue`, `paper_status`, `count`) VALUES ('$paper_id','$_SESSION[author_id]','','$paper_title','$paper_abstract','$paper_keyword','$paper_type','$input_field_name','$input_field_affiliation','$input_field_designation','$input_field_email','$manuscript_pdf_file_name','$cover_letter_pdf_file_name','$manuscript_image_file_name','$supplimentary_pdf_file_name','$volume','$issue','1','1')";
